...
async function submitLogin() {
    ...
    if (currentUser.must_change_password) {
        alert('يجب تغيير كلمة المرور الافتراضية. يمكنك ذلك من إعدادات المستخدمين.');
    }
...
async function switchTab(tab) {
    ...
    main.innerHTML = '';
    closeModal(); // إغلاق أي نافذة مفتوحة عند تغيير التبويب
    switch (tab) {
...
async function checkoutPOS() {
    ...
    for (let item of cart) {
        ...
        const recipe = (productResult.success && productResult.data) ? productResult.data.recipe || null : null;
        items.push({
            product_id: item.id,
            qty: item.qty,
            price: item.price,
    ...
    const result = await api.checkoutOrder(orderData);
    if (!result.success) {
...
async function openProductModal(id = null) {
    ...
    let recipeRows = '';
    try {
        const recipeArr = JSON.parse(prod.recipe || '[]');
        recipeRows = recipeArr.map(r => `
            <div style="display:flex; gap:5px; margin-bottom:4px;">
                <select class="mat-select">${cachedMaterials.map(m => `<option value="${m.id}" ${m.id==r.material_id?'selected':''}>${m.name} (${m.unit})</option>`).join('')}</select>
...
function addRecipeRow() {
    ...
    if (!cachedMaterials || cachedMaterials.length === 0) {
        alert('لا توجد مواد خام متاحة. أضف بعضها أولاً.');
        return;
    }
    const matOptions = cachedMaterials.map(m => `<option value="${m.id}">${m.name} (${m.unit})</option>`).join('');
    const div = document.createElement('div');
    div.style.cssText = 'display:flex; gap:5px; margin-bottom:4px;';
    div.innerHTML = `<select class="mat-select">${matOptions}</select> <input class="mat-qty" step="0.01" placeholder="كمية" style="width:80px;"> <button class="btn btn-danger btn-sm" onclick="this.parentElement.remove()">×</button>`;
...
// دوال المودال الخاصة بالتحليل المالي (اختصاراً تم إبقاؤها كما هي سابقاً)
async function openExpensesModal() { /* ... كما سبق ... */ }
...
// ... (باقي دوال المستخدمين والإعدادات والكيبورد كما كانت مع التحسينات الطفيفة) ...