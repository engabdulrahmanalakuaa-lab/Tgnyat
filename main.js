// main.js - محدّث مع إصلاحات الأخطاء
...
ipcMain.handle('create-user', (event, data) => {
    try {
        ...
        if (e.message.includes('UNIQUE constraint failed')) {
            return sendError(event, 'اسم المستخدم موجود بالفعل لهذه الشركة');
        }
...
// إنشاء شركة مع كلمة مرور افتراضية أقوى وإجبار على التغيير
ipcMain.handle('create-company', (event, { name }) => {
    try {
        ...
        const tempPassword = crypto.randomBytes(4).toString('hex'); // 8 أحرف عشوائية
        const salt = bcrypt.genSaltSync(10);
        const hashedPassword = bcrypt.hashSync(tempPassword, salt);

        db.prepare('INSERT INTO users (company_id, full_name, username, password_hash, role, must_change_password) VALUES (?,?,?,?,?,?)')
            .run(companyId, 'المدير العام', 'admin', hashedPassword, 'admin', 1);
        db.prepare('INSERT INTO settings (company_id) VALUES (?)').run(companyId);

        return sendSuccess({ id: companyId, name: name.trim(), tempPassword });
    } catch (e) {
...
// طباعة حرارية مع معالجة أخطاء التحميل
ipcMain.on('print-thermal', (event, htmlContent) => {
    ...
    try {
        ...
        printWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription) => {
            console.error('فشل تحميل صفحة الطباعة:', errorDescription);
            printWindow.close();
...
// نسخ احتياطي آمن باستخدام db.backup()
ipcMain.handle('backup-database', async () => {
    try {
        ...
        if (filePath) {
            // استخدام دالة النسخ الاحتياطي المدمجة
            await db.backup(filePath);
            return sendSuccess();
        }
        return sendSuccess({ cancelled: true });
    } catch (e) {
        return sendError(null, 'خطأ في النسخ الاحتياطي: ' + e.message);
    }
});
...
// ----------------- قناة إنهاء الطلب في معاملة واحدة -----------------
ipcMain.handle('checkout-order', (event, orderData) => {
    const {
        company_id, table_id, waiter_id, user_id, total, date, time, shift_id,
        items // مصفوفة من { product_id, qty, price, recipe (JSON string) }
    } = orderData;

    const transaction = db.transaction(() => {
        // إدراج الطلب
        const orderInfo = db.prepare('INSERT INTO orders (company_id, table_id, waiter_id, user_id, total, date, time, shift_id) VALUES (?,?,?,?,?,?,?,?)')
            .run(company_id, table_id, waiter_id, user_id, total, date, time, shift_id);
        const orderId = orderInfo.lastInsertRowid;

        // إدراج الأصناف وتحديث المخزون
        for (let item of items) {
            db.prepare('INSERT INTO order_items (order_id, product_id, qty, price) VALUES (?,?,?,?)')
                .run(orderId, item.product_id, item.qty, item.price);
            ...
            if (item.recipe) {
                ...
                try {
                    ...
                    throw new Error('وصفة غير صالحة للمنتج ' + item.product_id);
                }
                for (let comp of recipe) {
                    // خصم المخزون مع التحقق من الكفاية
                    const mat = db.prepare('SELECT current_stock FROM raw_materials WHERE id=? AND company_id=?').get(comp.material_id, company_id);
                    if (!mat) throw new Error('مادة خام غير موجودة');
                    if (mat.current_stock < comp.qty * item.qty) {
                        throw new Error(`المخزون غير كافٍ للمادة ${comp.material_id}`);
                    }
                    ...
                    db.prepare('INSERT INTO inventory_transactions (company_id, material_id, qty_change, type, reference, date) VALUES (?,?,?,?,?,?)')
                        .run(company_id, comp.material_id, -comp.qty * item.qty, 'consumption', `طلب #${orderId}`, date);
                }
            }
        }

        // تحديث الطاولة إذا وجدت
        if (table_id) {
    ...
    try {
        ...
        console.error('checkout-order error:', e.message);
        return sendError(event, 'فشل حفظ الطلب: ' + e.message);
    }
});